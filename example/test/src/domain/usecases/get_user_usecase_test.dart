...
void main() {
  test('Given getUserUseCase when Parameters user UUID does exist return successfull', () async {
  ...
  test('Given getUserUseCase when getUser and user UUID does exist return the namd and the age', () async {
    GetUserUseCase getUserUseCase;
    getUserUseCase = GetUserUseCase(MockGetUser());
    final testUser = await getUserUseCase.usersRepository.getUser('1000-2000-5600');
    expect(testUser.name, 'John');
    expect(testUser.age, 30);
  });
  test('Given getUserUseCase when Parameters user UUID does not exist return failed', () async {
    GetUserUseCase getUserUseCase;
    _Observer observer;
    getUserUseCase = GetUserUseCase(MockGetUser());
    observer = _Observer();
